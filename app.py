# app.py  – Groundwater dashboard with depth-oriented ANN plots
# ──────────────────────────────────────────────────────────────
...
def _fit_ann(df: pd.DataFrame, well: str, tst: float, layers: tuple[int, ...]):
    ...
    # ← FIX: assign train/test predictions separately (no tuple assignment)
    df.loc[Xtr.index, "pred"] = ytr_pred
    df.loc[Xte.index, "pred"] = yte_pred
    ...
    return mdl, sc, df, metrics

def _recur_forecast(mdl, sc, row, well, lags, horizon):
    rows, r = [], row.copy()
    for _ in range(horizon):
        ...
        X_next = sc.transform(r.drop(labels=[well, "Date"]).to_frame().T)
        val = mdl.predict(X_next)[0]
        r[well] = r["pred"] = val
        rows.append({"Date": nxt, "Forecast": val})
...
def ann_page():
    ...
    # depth plots (y-axis reversed)
    fig1 = px.line(dfp, x="Date", y=[well, "pred"],
                   labels={"value": "Water-table depth (m)", "variable": "Legend"},
                   title=f"{well} – fit")
    fig1.update_yaxes(autorange="reversed")
    st.plotly_chart(fig1, use_container_width=True)

    future = _recur_forecast(mdl, sc, dfp.tail(1).iloc[0], well, lags, horizon)
    fig2 = px.line(
        pd.concat([dfp[["Date", well]].rename(columns={well: "Depth"}),
                   future.rename(columns={"Forecast": "Depth"})]),
        x="Date", y="Depth",
        title=f"{well} – {horizon}-month forecast"
    )
...
def well_map_viewer():
    ...
    with t_filters:
        ...
        sub = st.multiselect("Select Sub-District(s):",
...
def groundwater_data_page():
    ...
    gw["Year"]   = gw["Year"].astype(int)
    gw["Months"] = gw["Months"].astype(int)
...
if page == "Home":
    ...
    st.markdown("Explore wells, clean datasets, and generate depth-oriented forecasts.")
