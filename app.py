# app.py — Groundwater Forecasting (SARIMA · LSTM · CNN-LSTM)
# -----------------------------------------------------------
# • Upload a CSV with columns: Year, Months, W1…Wn
# • Choose a well + model → 60-month forecast table
# • “Save yearly summary” writes/updates yearly_summaries.csv
#
#   Runtime requirements (CPU wheels exist for Python ≤ 3.11):
#     streamlit  pandas  numpy  plotly
#     scikit-learn  statsmodels  tensorflow-cpu
#
...
def to_supervised(arr: np.ndarray, n_lags: int):
    X, y = [], []
...
def deep_forecast(series, horizon, n_lags=12, epochs=30,
                  model_type="lstm", batch=16, patience=5):
    scaler = MinMaxScaler()
...
if raw is None:
    st.error("CSV not found. Upload groundwater file.")
...
if len(series) < 36:
    st.warning("Need ≥36 monthly points for these models."); st.stop()

# Extra sliders for NN models
if model_choice.startswith(("LSTM", "CNN")):
    n_lags = st.sidebar.slider("Number of lags (months)", 6, 24, 12, step=2)
...
if st.button("💾 Save yearly summary"):
    ...
    if os.path.exists(SUMMARY_CSV):
        pd.concat([pd.read_csv(SUMMARY_CSV), new_row],
                  ignore_index=True).to_csv(SUMMARY_CSV, index=False)
    else:
        new_row.to_csv(SUMMARY_CSV, index=False)

    st.session_state.setdefault("summary_rows", []).append(new_row)
