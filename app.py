# app.py — Groundwater forecasting (ANN + ARIMA) with per-well clipping
# --------------------------------------------------------------------
# New feature ➜ every forecast is clipped to a *reasonable* band:
#       lower = min_depth − 20 % of historical range   (floor at 0 m)
#       upper = max_depth + 20 % of historical range
# Keeps predictions from jumping to “1 M” or −∞ while preserving realism.
...
def clean_series(df, well):
    """Remove ±3×IQR spikes & interpolate; add cyclical month encodings."""
    s = df[well].copy()
...
def clip_bounds(series):
    """Return (lower, upper) = [min-20%, max+20%] of range."""
    lo, hi = series.min(), series.max()
    rng = hi - lo if hi > lo else max(hi, 1)
    lower = max(0, lo - 0.2*rng)  # depth can't be negative
    upper = hi + 0.2*rng
    return lower, upper

# ─── ANN ───
def train_ann(df_feat, well, layers, lags, scaler_type, lower, upper):
    ...
    df_feat.loc[Xtr.index,"pred"] = np.clip(mdl.predict(Xtr_s), lower, upper)
    df_feat.loc[Xte.index,"pred"] = np.clip(mdl.predict(Xte_s), lower, upper)
    ...
    for _ in range(FORE_HORIZON):
        ...
        val_raw = mdl.predict(scaler.transform(r[feats].to_frame().T))[0]
        val = np.clip(val_raw, lower, upper)
        r[well]=r["pred"]=val; rows.append({"Date":nxt,"Depth":val})
    return metr, df_feat, pd.DataFrame(rows)

# ─── ARIMA ───
def train_arima(series, seasonal, lower, upper):
    mdl = ARIMA(series, order=(1,1,1),
    ...
    future = pd.DataFrame({"Date":pd.date_range(series.index[-1]+pd.DateOffset(months=1),
                           ...
                           "Depth":np.clip(fc.predicted_mean.values, lower, upper)})
    return {"AIC":round(mdl.aic,1),"BIC":round(mdl.bic,1)}, mdl, future
...
if model=="🔮 ANN":
    ...
    if len(clean) < lags*10:
        lags = max(1,len(clean)//10); st.info(f"Lags auto-reduced to {lags}")
    ...
    metr, hist, future = train_ann(feat, well, layers, lags, scaler_choice, lo, hi)
    ...
    metr, res, future = train_arima(series, seasonal, lo, hi)
