# app.py — Groundwater forecasts (ANN with clipping + ARIMA with RMSE_test)
# ------------------------------------------------------------------------
...
def train_ann(df_feat, well, layers, lags, scaler_type, lo, hi):
    ...
    mdl=MLPRegressor(hidden_layer_sizes=layers,max_iter=2000,
                     random_state=42,early_stopping=True).fit(
                     scaler.fit_transform(Xtr), ytr)
    ...
    metrics={"RMSE test":round(np.sqrt(mean_squared_error(
                    yte,df_feat.loc[Xte.index,"pred"])),4)}
    # recursive forecast (clipped)
...
def train_arima(series, seasonal, lo, hi):
    ...
    # refit on full series for 5-yr forecast
    ...
    metrics={"AIC":round(res_full.aic,1),
...
if model=="🔮 ANN":
    ...
    if len(clean)<lags*10: lags=max(1,len(clean)//10); st.info(f"Lags → {lags}")
    ...
    metr,res,future=train_arima(ser,seasonal,lo,hi)
...
st.subheader(f"{model.strip()} metrics"); st.json(metr)
