# app.py — Groundwater Forecasting (SARIMA · LSTM · CNN-LSTM, TF-optional)
# -----------------------------------------------------------------------
...
#                  scikit-learn==1.4.2
#                  statsmodels==0.14.4
#                  tensorflow-cpu==2.15.0      (# comment out if you skip TF)
# -----------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Optional TensorFlow import (lets the app run even when TF isn’t available)
# ------------------------------------------------------------------------------
try:
...
# ----------------------------- Deep helpers (only if TF exists) ----------------
if _TF_AVAILABLE:
...
# Model selector with availability labels
model_labels = [
...
if model_choice.startswith(("LSTM", "CNN")):
    if not _TF_AVAILABLE:
        st.error("TensorFlow isn’t installed, so deep-learning models are unavailable.\n"
                 "Add `tensorflow-cpu==2.15.0` to requirements.txt and pin python-3.11 "
                 "in runtime.txt, then redeploy.")
        st.stop()
